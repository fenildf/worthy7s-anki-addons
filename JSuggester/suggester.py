...
u"""Main file of the Japanese word suggester."""

from PyQt4.QtCore import Qt, SIGNAL
from PyQt4.QtGui import QAction, QApplication, QComboBox, QGridLayout, \
    QLabel, QLineEdit, QListWidget, QListWidgetItem, QPushButton, QWidget
...
class WordSuggestor(QWidget):
    u"""
    The word suggester widget.
    """
    ...
    def initUI(self):
        u"""Set up the user interface"""
        #Create elements
        self.optionsLabel = QLabel(
            'Search for cards as you do with the card browser')
        self.optionsLabel.setAlignment(Qt.AlignCenter)
        self.kanjiCardsLabel = QLabel('Kanji Cards Search:')
        self.kanjiCardsSearch = QLineEdit('Note:"Japanese (Kanji)"')
        self.kanjiCardsLabel.setAlignment(Qt.AlignCenter)
        self.kanjiCardsFieldLabel = QLabel('Select Field: ')
        self.kanjiCardsFieldLabel.setAlignment(Qt.AlignCenter)
        self.kanjiCardsFieldSelector = QComboBox()
        #self.kanjiCardsResultsLabel = QLabel('Found: ')
        self.kanjiCardsButton = QPushButton('Get cards!')
        self.kanjiCardsResultsNumberLabel = QLabel('0')
        self.WordCardsLabel = QLabel('Word Cards Search:')
        self.WordCardsLabel.setAlignment(Qt.AlignCenter)
        self.WordCardsSearch = QLineEdit('Note:"Japanese (Vocab)"')
        # self.WordCardsResultsLabel = QLabel('Found: ')
        self.WordCardsButton = QPushButton('Get cards!')
        self.WordCardsResultsNumberLabel = QLabel('0')
        self.WordCardsFieldLabel = QLabel('Select Field: ')
        self.WordCardsFieldLabel.setAlignment(Qt.AlignCenter)
        self.WordCardsFieldSelector = QComboBox()
        self.resultsList = QListWidget()
        self.ignoreButton = QPushButton('Ignore Selected')
        ...
        # self.kanjiCardsSearch.editingFinished.connect(
        #     self.onKanjiFieldChange)
        # self.WordCardsSearch.editingFinished.connect(
        #     self.onVocabFieldChange)
    ...
    def loadElementsPickle(self):
        ...
        if os.path.exists(elementsPicklePath):
            ...
            kanjiFIndex = self.kanjiCardsFieldSelector.findText(
                loadedPickle["kanjiSelectedFieldText"])
            wordFIndex = self.WordCardsFieldSelector.findText(
                loadedPickle["kanjiSelectedFieldText"])
            if kanjiFIndex < 0:
                kanjiFIndex = 0
            if wordFIndex < 0:
                wordFIndex = 0
            # might cause an error if field doesnt exist anymore (get
            # index return 0?)
    ...
    def saveElementsPickle(self):
        ...
        savePickle = {
            ...
            "kanjiSelectedFieldText":
                self.kanjiCardsFieldSelector.currentText(),
    ...
    def getKanjiList(self):
        ...
        if __name__ == '__main__':
            ...
            self.kanjiCardsResultsNumberLabel.setText(
                str(len(self.knownKanjis)))
            return
        #########
        #get cards from anki
        ...
        for cardid in self.knownKanjiCardIDs:
            try:
                ...
                kanji = re.findall(
                    ur'[\u4e00-\u9fbf]', unicode(note[kanjiField]))
                if not kanji:
                    continue
                else:
                    kanji = kanji[0]
                # possible problem if kanji is already in this list?
                # what do? for now overwrite
    ...
    def getWordList(self):
        ...
        if __name__ == '__main__':
            ...
            self.WordCardsResultsNumberLabel.setText(
                str(len(self.knownKanjis)))
            return
        #########
        #goto anki etc
        ...
        for cardid in self.knownWordCardIDs:
            try:
                card = mw.col.getCard(cardid)
                note = card.note()
                # get bit before the reading, if there is any. Else
                # get the whole field
                ...
                if word:
                    ...
                    #strio html
                    word = stripHTML(word)
                # just set boolean, we're only checking to see if the
                # word exists
    ...
    def onSearch(self):
        ...
        #     'SELECT * FROM dict WHERE kanji NOT IN ignoreWords').fetchall()
        # get dict and update values, then sort and return results
        # get initial results excluding known words and ignore words
        # results = []
        resultKeys = self.edict.viewkeys() \
            - (self.knownWords.viewkeys() | self.ignoredWords.viewkeys())
...
#             count += 1
            try:
                ...
                kanjiMod = 0.5
            self.edict[r]['sortValue'] = \
                self.edict[r]['percentOfTotal'] * kanjiMod
        # sort dict again
        ## slow implementation apparently?
        # sorted(results, key=results.get)
        ## faster more confusing implementation?
        sorted_results = sorted(
            resultKeys, key=lambda k: self.edict[k]['sortValue'], reverse=True)
        # clear list
    ...
    def populateListFromPickle(self, results):
        # entry = {
        #     'reading':reading, 'meaning':meaning, 'pFlag':popular,
        #     'kanjis':kanjis, 'percentOfTotal':percentOfTotal}

        for i in results:
            # item = QListWidgetItem(
            #     unicode(str(self.edict[i]['sortValue']) + '::' + i
            #             + '[' + self.edict[i]['reading'] + ']: '
            #             + self.edict[i]['meaning']))
            item = QListWidgetItem(
                unicode(i + '[' + self.edict[i]['reading'] + ']: '
                        + self.edict[i]['meaning']))
            item.word = unicode(i)
    ...
    def onSearchDB(self):
        ...
        # ignoreWords += [
        #     entry.decode('UTF-8') for entry in  self.ignoredWords.keys()]
...
#              self.cursor.execute(
#                  'INSERT INTO ignoreWords (word) VALUES (?)', [ent])
        self.cursor.executemany(
            'INSERT INTO ignoreWords (word) VALUES (?)', ignoreWordsTupled)
        # some sql string formatting
        # sql="SELECT * FROM dict WHERE kanji NOT IN ({seq}) LIMIT
        # 20".format(seq=','.join(['?']*len(ignoreWords))) results =
        # self.cursor.execute(sql, ignoreWords).fetchall()
        results = self.cursor.execute(
            'SELECT * FROM dict WHERE kanji NOT IN ignoreWords').fetchall()
        # search DB for top 20 words that are not in ignore and not in
        # knownwords and have kanji in the knownKanjis list close
        self.cursor.close()
    ...
    def createDictionaryDatabase(self):
        ...
        self.cursor.execute(
            '''CREATE TABLE dict(
kanji TEXT, kana TEXT, entry TEXT, p INT, kanjis TEXT , ignore INT)''')
...
#             4\t[terms and definitions delimited by pipes]
            ...
            for i in reader:
                ...
                if len(meaningsCount) > 1:
                    ...
                    # (expression, unicode(), glossary, conjugations,
                    # source, count)
                    if not bestResult:
                ...
                if popular:
                    ...
                    popular = 0
                into_db.append(
                    (expression, reading, meaning, popular,
                     unicode(' '.join(kanjis)), '0'))
            print into_db[0::20]
            self.cursor.executemany(
                """INSERT INTO dict(
kanji , kana , entry , p , kanjis , ignore) VALUES (?, ?, ?, ?, ?, ?);""",
                into_db)
...
#             4\t[terms and definitions delimited by pipes]
            ...
            for i in reader:
                ...
                if popular:
                    ...
                    popular = 0
                    #{'one': 1, 'two': 2, 'three': 3}
                # percentOfTotal, expression, reading , meaning,
                # popular, kanjis[])
                entry = {
                    'reading': reading, 'meaning': meaning,
                    'pFlag': popular, 'kanjis': kanjis,
                    'percentOfTotal': float(percentOfTotal)}
    ...
    def getBestMeaning(self, word):
        ...
        for i in range(0, len(results)):
            results[i] = results[i] \
                + (unicode(japanese_examples.howManyExamples(results[i][0])),)
        #build unique dict
        resultsByKey = dict()
        for r in results:
            resultsByKey[r[0] + ' ' + r[1] + ' ' + r[2] + ' ' + r[3]
                         + ' ' + r[4] + ' ' + r[5]] = r
...
#         self.createDictionaryDatabase()
        ...
        if not os.path.exists(edictPicklePath) and os.path.exists(edictPath):
            # if no pickle, but there is an edict file, read from
            # EDICT, and create the database
